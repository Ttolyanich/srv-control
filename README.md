...
> **Доступ к API синхронизации.** Эндпоинты `/api/sync/<id>` и `/api/sync/all` больше не доверяют запросам с `127.0.0.1` (за reverse-proxy так выглядят все запросы). Штатная автосинхронизация работает через встроенный планировщик внутри приложения и в этих эндпоинтах не нуждается. Из веб-интерфейса под учеткой admin/financier всё работает как раньше. Если когда-нибудь понадобится дернуть синхронизацию внешним скриптом — передавайте токен: `curl -H "X-Sync-Token: $(cat instance/sync_token)" http://127.0.0.1:5002/api/sync/all`.
